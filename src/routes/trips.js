...
const fareData = await calculateFare({ serviceType: tripRequest.serviceType, distanceKm: 0, durationMinutes: 0 });
...
'driverProfile.currentTripId': newTrip[0]._id
      }, { session });
    });
...
res.json({ success: true, message: 'Trip accepted and created' });
  } catch (err) {
next(err);
  }
});
...
if (trip.status !== 'pending') return res.status(400).json({ error: { message: 'Trip not in pending state' } });
trip.status = 'started';
...
if (['completed', 'cancelled'].includes(trip.status)) return res.status(400).json({ error: { message: 'Trip already finished' } });
...
emitter.emit('notification', { userId: trip.passengerId, type: 'trip_completed', title: 'Trip completed (cash)', body: `Pay driver ₦${Math.floor(trip.finalFare / 100)}` });
emitter.emit('notification', { userId: driverId, type: 'trip_completed', title: 'Confirm cash', body: 'Confirm collection via /confirm-cash' });
...
if (commission > 0) {
await Transaction.create({ userId: null, type: 'commission', amount: commission, reference: trip._id, status: 'success' });
    }
trip.status = 'completed';
trip.completedAt = new Date();
await trip.save();
...
emitter.emit('notification', { userId: trip.passengerId, type: 'trip_completed', title: 'Trip completed', body: `Charged ₦${Math.floor(trip.finalFare / 100)}` });
emitter.emit('notification', { userId: driverId, type: 'trip_completed', title: 'Earnings', body: `You earned ₦${Math.floor(trip.driverEarnings / 100)}` });
...
if (!isParticipant && !isAdmin) return res.status(403).json({ error: { message: 'Unauthorized' } });
res.json({ trip });
  } catch (err) {
next(err);
  }
...
const driverId = req.user.sub;
const trip = await Trip.findById(req.params.tripId);
if (!trip || trip.driverId.toString() !== driverId) return res.status(403).json({ error: { message: 'Unauthorized' } });
if (trip.paymentMethod !== 'cash') return res.status(400).json({ error: { message: 'Not a cash trip' } });
const pendingTx = await Transaction.findOne({ reference: trip._id, type: 'trip_payment', status: 'pending' });
if (!pendingTx) return res.status(404).json({ error: { message: 'No pending cash transaction' } });
...
await Transaction.create({ userId: null, type: 'commission', amount: trip.commission, reference: trip._id, status: 'success' });
    }
